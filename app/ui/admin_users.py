...
def admin_users_page() -> None:
            ...
            for u in users:
                with ui.card().classes("w-full"):
                    ...
                    with ui.row().classes("gap-2"):
                        ui.button("Сменить PIN", on_click=lambda uid=u.id, n=u.name: _change_pin(uid, n)).props("flat")
                        if u.is_active:
    ...
    def _change_pin(user_id: int, name: str) -> None:
        with ui.dialog() as dialog, ui.card().classes("gap-3"):
            ...
            pin_in = ui.input("PIN (4–6 цифр)", password=True).props("inputmode=numeric")
    ...
    with ui.expansion("Добавить пользователя").classes("w-full max-w-2xl mt-2"):
        ...
        new_pin = ui.input("PIN (4–6 цифр)", password=True).props("inputmode=numeric").classes("w-full")
