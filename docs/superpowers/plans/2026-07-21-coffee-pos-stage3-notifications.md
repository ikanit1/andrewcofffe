...
- **Бэклог (найдено финальным ревью ветки, первый по приоритету для этапа 4):**
  если админ ни разу не нажал `/start` (или у бота нет ни одного достижимого админа),
  `bot.send_message` для него будет отклонён Telegram, запись `NotificationOutbox`
  никогда не пометится `sent` и будет ретраиться каждые 5 секунд бессрочно — очередь
  растёт без ограничения, а при первом реальном контакте админ получит разом весь
  накопленный бэклог событий. Решение: TTL/ограничение возраста для ретраев, либо
  при первом успешном `/start` помечать старые накопленные записи как `sent`/`skipped`
  вместо доставки всех разом. Также стоит свести `logger.exception` на каждый тик при
  полном отсутствии достижимых админов к одному периодическому предупреждению.
